...
import swave.core.impl.{ InportList, Outport, Inport, StartContext }
...
private[core] final class FlattenConcatStage(streamable: Streamable.Aux[AnyRef, AnyRef],
  ...
  def running(ctx: StartContext, in: Inport, out: Outport) = {
    ...
    def awaitingSub(remaining: Long): State =
      state(name = "awaitingSub",
        ...
        onNext = (elem, _) ⇒ {
          ...
          sub.subscribe()
          awaitingOnSubscribe(sub, remaining)
        },
     ...
     * @param remaining   number of elements already requested by downstream but not yet delivered, >= 0
     */
    def awaitingOnSubscribe(subscribing: Inport, remaining: Long): State =
      state(name = "awaitingOnSubscribe",
        ...
        request = (n, _) ⇒ awaitingOnSubscribe(subscribing, remaining ⊹ n),
     ...
     * @param remaining   number of elements already requested by downstream but not yet delivered, >= 0
     */
    def active(subCount: Int, subscribing: Inport, subscribed: InportList, pending: Long, remaining: Long): State =
      state(name = "active",
          ...
          if (from ne in) {
            ...
            if (subCount > 1) {
              if (from eq subscribed.in) { // if the current sub completed
                ...
                } else { requireNonNull(subscribing); awaitingOnSubscribe(subscribing, remaining) }
