...
private[core] final class InjectSequentialBufferedStage(bufferSize: Int) extends InOutStage { stage =>

  def kind = Stage.Kind.Inject.Sequential

  private[this] var buffer: RingBuffer[AnyRef] = if (bufferSize > 0) createBuffer(bufferSize) else null

  private def createBuffer(size: Int) = new RingBuffer[AnyRef](roundUpToPowerOf2(size))
