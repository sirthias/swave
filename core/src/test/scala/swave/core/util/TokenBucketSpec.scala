...
class TokenBucketSpec extends FreeSpec with Matchers {
  ...
  "A Token Bucket should" - {
    ...
    "start full" in {
      ...
      bucket.offer(1) shouldEqual 0
      bucket.offer(1) shouldEqual 0
      bucket.offer(1) shouldEqual 0
      bucket.offer(7) shouldEqual 0

      bucket.offer(3) shouldEqual 3
    }

    "calculate correctly with different rates and capacities" in {
      val bucketRate2 = new TestBucket(10, 2)

      bucketRate2.offer(5) shouldEqual 0
      bucketRate2.offer(5) shouldEqual 0
      bucketRate2.offer(5) shouldEqual 10

      val bucketRate3 = new TestBucket(8, 3)
      bucketRate3.offer(5) shouldEqual 0
      bucketRate3.offer(5) shouldEqual 6

      bucketRate3.currentTime = 6
      bucketRate3.offer(3) shouldEqual 9
    }

    "allow sending elements larger than capacity" in {
      val bucket = new TestBucket(10, 2)

      bucket.offer(5) shouldEqual 0
      bucket.offer(20) shouldEqual 30

      bucket.currentTime = 30
      bucket.offer(1) shouldEqual 2
    ...
    "work with zero capacity" in {
      ...
      bucket.offer(10) shouldEqual 20
    ...
    "maintain maximum capacity" in {
      val bucket = new TestBucket(10, 1)
      bucket.offer(10) shouldEqual 0

      bucket.currentTime = 100000
      bucket.offer(20) shouldEqual 10
    }

    "work if currentTime is negative" in {
      val bucket = new TestBucket(10, 1)
      bucket.currentTime = -100
      bucket.reset()

      bucket.offer(5) shouldEqual 0
      bucket.offer(10) shouldEqual 5
    ...
    "work if currentTime wraps around" in {
      val bucket = new TestBucket(10, 1)
      bucket.currentTime = Long.MaxValue - 5
      bucket.reset()
    ...
    "(attempt to) maintain equal time between token renewal intervals" in {
      ...
      bucket.offer(10) shouldEqual 15
      ...
      //
      bucket.offer(2) shouldEqual 5

      // another case
      val bucket2 = new TestBucket(10, 3)

      bucket2.currentTime = 4
      bucket2.offer(6) shouldEqual 0
      ...
      for {
        ...
        for (time ← 0 to 1000) {
          ...
          if (untilNextElement == 0) {
            ...
            val delay = bucket.offer(cost.toLong)
            nextEmit = time + delay
