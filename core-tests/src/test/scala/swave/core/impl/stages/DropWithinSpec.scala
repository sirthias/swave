...
final class DropWithinSpec extends SwaveSpec {
  ...
  "DropWithin" in {
    ...
    stream.dropWithin(20.millis).drainTo(drain) shouldEqual Try(())

    drain.send.request(5)
    stream.expect.request(5) within 5.millis
    stream.send.onNext('a)
    stream.expect.request(1) within 5.millis
    ...
    drain.expect.onNext('d) within 5.millis
    stream.send.onComplete()
    drain.expect.onComplete() within 5.millis
  }
}